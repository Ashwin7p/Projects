...
for row in cur:
    if row[0]==uname:
        if row[1]==password:
            ...
            print('<html>')
            print('<head>')
            print('<meta http-equiv="Refresh" content="0;url='+str(redirectURL)+'" >') #metarefresh option(its a method to tell  web browser) used as page is                                                                                  refreshing redirection is possible
            print('</head>')
            print('</html>')
            flag=1
            break
